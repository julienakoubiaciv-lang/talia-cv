...
describe('mergeProgress', () => {
  it('applique le bon fusionneur par clé', () => {
    const local = {
      altio_player: { xp: 100, badges: { a: 10 } },
      altio_recruit_progress: { bestNote: 8, plays: 1 },
      altio_letters_count: 3,
    };
    const remote = {
      altio_player: { xp: 300, badges: { b: 20 } },
      altio_recruit_progress: { bestNote: 14, plays: 2 },
      altio_letters_count: 5,
      altio_codes_progress: { bestNote: 18, plays: 4 },
    };
    const m = mergeProgress(local, remote);
    expect(m.altio_player.xp).toBe(300);
    expect(m.altio_player.badges).toEqual({ a: 10, b: 20 });
    expect(m.altio_recruit_progress).toEqual({ bestNote: 14, plays: 2 });
    expect(m.altio_letters_count).toBe(5);
    expect(m.altio_codes_progress).toEqual({ bestNote: 18, plays: 4 }); // clé seulement distante
  });

  it('clé présente d\'un seul côté est conservée', () => {
    expect(mergeProgress({ altio_letters_count: 2 }, {})).toEqual({ altio_letters_count: 2 });
    expect(mergeProgress({}, { altio_letters_count: 4 })).toEqual({ altio_letters_count: 4 });
  });
});

describe('progressStats', () => {
  it('extrait xp et série du profil', () => {
    expect(progressStats({ altio_player: { xp: 420, dayStreak: 7 } })).toEqual({ xp: 420, day_streak: 7 });
  });
  it('blob vide → zéros', () => {
...
describe('snapshotLocal / hydrateLocal (round-trip)', () => {
  ...
  it('lit les clés présentes en respectant les types', () => {
    localStorage.setItem('altio_player', JSON.stringify({ xp: 50, badges: {} }));
    localStorage.setItem('altio_letters_count', '4');
    const snap = snapshotLocal();
    expect(snap.altio_player).toEqual({ xp: 50, badges: {} });
    expect(snap.altio_letters_count).toBe(4);
    expect('altio_oral_progress' in snap).toBe(false);
  });

  it('hydrate réécrit le cache (objets en JSON, nombres en chaîne)', () => {
    hydrateLocal({ altio_player: { xp: 99 }, altio_letters_count: 7 });
    expect(JSON.parse(localStorage.getItem('altio_player'))).toEqual({ xp: 99 });
    expect(localStorage.getItem('altio_letters_count')).toBe('7');
  });

  it('round-trip snapshot → hydrate → snapshot est stable', () => {
    const blob = { altio_player: { xp: 12, dayStreak: 2, lastPlay: '2026-06-09', badges: { a: 1 } }, altio_codes_progress: { bestNote: 15, plays: 3 }, altio_letters_count: 6 };
    hydrateLocal(blob);
